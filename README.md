...
- Single strategic map state with units, owners, turn state, selectable units, reachable movement, ZOC flags, integer-scaled movement costs, horde resource counters, and the first horde-to-herd resource action. The first combat units are cavalry, infantry, and horde; herds are the first movable resource unit.
