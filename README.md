...
Rivers are emitted as first-class hydrology records with northern sources, optional steppe merge points, southern destinations, edge-only river segments, and deduplicated river edges. Temporary hex paths are used only inside generation and are not part of the normal output.
