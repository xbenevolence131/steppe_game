...
- Forest blobs are derived after roads and the eastern desert footprint, then seed from wild terrain adjacent to grassland and grow as seeded blobby frontier expansions. `Forest blobs` defaults to `4`, valid range is `0-10`, and `0` disables the blobs; `Forest radius` defaults to `4`, valid range is `0-20`, and `0` also disables them. They can cover grassland and wild terrain but not lakes, desert, towns, or road paths.
...
- Roads route from hex center to hex center and are emitted in top-level `roads` as coordinate paths. The browser draws regional roads by owner color and draws `silk_road` as a thicker gold-brown trunk. Current roads connect Persian towns and Chinese towns in nearest-neighbor regional chains. A `dzungarian_gate` town is placed at the central seeded pass in the pinch, an `oasis` town is placed on a river-adjacent grassland/valley hex near the middle of the map, and the `silk_road` routes from the easternmost Persian town through the Dzungarian gate and oasis to the westernmost Chinese town while avoiding lakes and wild terrain. Road routing uses a generic A* hex pathfinder; a final conservative cleanup pass straightens short zigzagging road chunks without changing road endpoints or allowing illegal terrain.
