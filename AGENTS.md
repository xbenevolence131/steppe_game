...
- Non-steppe, non-valley land is filled as wild terrain instead of left empty: hexes near grassland skew toward woods and hills, while farther hexes increasingly become mountains.
- Generated hexes include a `labels` array preserving generation provenance. Current labels include `base_steppe`, `base_none`, `lake`, `valley`, and `wild_terrain`; hills, mountains, and woods from the non-steppe fill should retain `wild_terrain`.
