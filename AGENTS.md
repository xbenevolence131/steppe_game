...
- Current combat-capable unit kinds are horse-archers, infantry, and horde. Horse-archers have move `4`, infantry has move `2`, and horde has move `3`. Horse-archers, infantry, and horde project ZOC; infantry and horde respect enemy ZOC, while horse-archers currently ignore it.
- Each horde carries non-negative integer resource counters: `population`, `metal`, and `horses`. These serialize on units and are shown in the unit inspector only when a horde is selected.
- The first resource action is `detach_herd`: active-faction hordes with `horses > 0` may detach horses into a new adjacent herd. The browser shows the amount popover and deployment highlights, but C++ validates the amount, destination, horde ownership, occupancy, and terrain before mutating state.
...
- In play mode, a resource status bar sits above the sidebar/map workbench and sums the active faction's horde `population`, `horses`, and `metal`.
