...
- Generated hexes include a `labels` array preserving generation provenance. Current labels include `base_steppe`, `base_none`, `lake`, `valley`, `wild_terrain`, and `forest_blob`; hills, mountains, and forest from the non-steppe fill should retain `wild_terrain`.
